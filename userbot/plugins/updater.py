...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**Master, Updates are Available For {ac_br} Branch, Changelogs**\n{changelog}"
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if build.status == "failed":
            await event.edit(f"`Build failed!`\n[Click here](https://dashboard.heroku.com/apps/{heroku_app}/logs) `to Check for Logs...`")
